...
INPUT:
    raw_data/<trial_name>/calibration.json     — pixel scale + source boundary polygon
    raw_data/<trial_name>/Results/*_masks.npy  — Cellpose instance segmentation masks

OUTPUT (written to raw_data/aggregate_edt/):
    <trial>_edt_droplets.csv      — per-droplet per-frame: time, radius, EDT distance
    <trial>_edt_binned_statistics.csv — binned summary (spatial bins × time)
    <trial>_boundary_polygon.csv  — source boundary polygon in µm

USAGE:
    python step1_batch_process.py
    python step1_batch_process.py --trial 2to1.4   # process one trial only

ADAPTING TO OTHER TRIALS:
    Modify TRIAL_CONFIG at the bottom to add new trial folders.
    For hydrogels: set 'source_type' = 'ellipse'
    For fungi: set 'source_type' = 'polygon'

NPY FILE LOCATION:
    NPY files are Cellpose segmentation outputs (~20 MB each, ~1 GB per trial).
    They are stored on external drive T7 — see AllTrials/README.txt for paths.
    Copy the Results/ folder from T7 to raw_data/<trial_name>/Results/ before running.
"""
...
# ─── Trial configuration ───────────────────────────────────────────────────────
# For each trial: folder name inside raw_data/, and the subfolder with NPY files
# source_type: 'ellipse' for hydrogels, 'polygon' for fungi
TRIAL_CONFIG = {
    # ── Green fungi ──
    'Green.1':     {'folder': 'Green.1',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'Green.2':     {'folder': 'Green.2',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'Green.3':     {'folder': 'Green.3',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'Green.3_new': {'folder': 'Green.3_new', 'npy_subdir': 'Results', 'source_type': 'polygon'},
    'Green.4':     {'folder': 'Green.4',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'Green.5':     {'folder': 'Green.5',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    # ── White fungi ──
    'white.1':     {'folder': 'white.1',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'white.3':     {'folder': 'white.3',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'white.4':     {'folder': 'white.4',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'white.5':     {'folder': 'white.5',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'white.6':     {'folder': 'white.6',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    # ── Black fungi ──
    'Black.2':     {'folder': 'Black.2',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'black.1':     {'folder': 'black.1',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'black.3':     {'folder': 'black.3',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'black.4':     {'folder': 'black.4',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'black.5':     {'folder': 'black.5',     'npy_subdir': 'Results', 'source_type': 'polygon'},
    'black.new':   {'folder': 'black.new',   'npy_subdir': 'Results', 'source_type': 'polygon'},
    'black.new2':  {'folder': 'black.new2',  'npy_subdir': 'Results', 'source_type': 'polygon'},
}
...
def process_frame(npy_path: Path, time_sec: float, pixel_size: float,
    ...
    """Extract per-droplet measurements from one frame."""
...
def compute_binned_statistics(droplet_df: pd.DataFrame) -> pd.DataFrame:
    """Compute mean radius per (time, distance bin)."""
