#!/usr/bin/env python3
"""
step2_beysens_profile.py
========================
Compute Beysens growth profiles and generate per-trial figures.

Framework (Beysens 2006):
    ⟨R⟩(t) ∝ t^β   where β ≈ 1/3 (diffusion-limited) or ≈ 1 (coalescence)
    Surface coverage ε = Σπ·Rᵢ² / A_bin

INPUT:
    raw_data/aggregate_edt/<trial>_edt_droplets.csv
    raw_data/aggregate_edt/<trial>_boundary_polygon.csv

OUTPUT (written to output/):
    beysens_<trial>.png / .pdf  — 5-panel per-trial analysis figure
    beysens_profile_results.csv — per-bin β, A, R², n_frames across all trials

USAGE:
    python step2_beysens_profile.py
    python step2_beysens_profile.py --trial 2to1.4

ADAPTING TO OTHER TRIALS:
    Modify TRIALS dict to add new trial_id: ('subfolder', delta_um) entries.
    delta_um = dry-zone radius in µm (from compute_metrics or literature).
"""
...
MIN_DROPLETS_BIN = 5
MIN_FRAMES_FIT   = 5
T_DATA_MIN       = 5.0    # start of analysis window (min)
T_DATA_MAX       = 15.0   # end of analysis window (min)

# ─── Trial registry ────────────────────────────────────────────────────────────
# trial_id: delta_um (dry-zone radius in µm; 0 for fungi since EDT is from polygon)
TRIALS = {
    '2to1.4': 900,
    # Add more trials:
    # 'agar.2':  100,
...
def compute_profile(droplets, delta_um, P_body):
    """Compute β(r′) and ε(r′) profiles via OLS log-log fit."""
    ...
    for j, bc in enumerate(bin_centers):
        ...
        # OLS log-log fit: log R = β·log t + log A
...
def plot_trial(profile, ts, tid, delta_um, out_path):
    ...
    # Panels 0–2: R(t) for near/mid/far
    ...
    # Panel 3: instantaneous β(t) overlaid for near/mid/far
    ax = axes[3]
    ...
    # Panel 4: all bins R(t) colored by distance (plasma)
    ax = axes[4]
...
def main():
    ...
    for tid, delta_um in trials.items():
        ...
        if P_body is None:
            # Fallback: estimate perimeter from a circle of radius delta_um
